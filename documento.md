...
- [x] Adicionar `loading="lazy"` e `decoding="async"` nas `<img>` dos cards.
- [x] Adicionar `revokeObjectURL()` nas blob URLs (liberar memória ao trocar/remover poster) e liberar os Blobs do IndexedDB ao remover/limpar/importar dados (`StoreImages.remove/clear/prune`).
- [x] Smoke test → VERDE (11/11) + teste novo 11 (lazy/async nos cards e remoção libera o Blob do IDB).
...
| 2026-08-14 | Teste 05 flaky (1 falha em ~20 execuções na suíte completa): `abrirInfoDoCard` estourava timeout em `#context-menu button` hasText `Info` ("element is not visible") mesmo com `#context-menu.show` visível | Falha rara de timing do clique sintético do Playwright com o fade-out/close do menu (`closeCtxMenu` remove `.show` e faz `display:none` após 200ms): entre o `waitFor` e o clique, o menu fechava. Não relacionado à Etapa 4 (o mesmo helper passou em 8/8 execuções isoladas e o fluxo de contexto não foi alterado). Causa exata não reproduzida | Helper `abrirInfoDoCard` robusto: tenta abrir o menu até 3× (re-abre o menu e limpa estado se o clique/modal não completar), mantendo a asserção real (modal de info abre). Se o menu não abrir, a tentativa falha e o teste quebra (não mascara regressão) | `tests/helpers.js` (abrirInfoDoCard) | 11/11 VERDE |
